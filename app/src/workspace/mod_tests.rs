...
use crate::workspace::view::{
    JUMP_TO_NEXT_UNREAD_TAB_BINDING_NAME, TOGGLE_ACTIVE_TAB_STAR_BINDING_NAME,
    TOGGLE_ACTIVE_TAB_UNREAD_BINDING_NAME,
};

/// ⌃⌘U, ⌃⌘S and ⌘J each belong to exactly one default binding: the tab mark
/// each was chosen for. They're macOS defaults only, so elsewhere nothing
/// binds them.
#[test]
    ...
    App::test((), |mut app| async move {
        ...
        app.update(|ctx| {
            for (key, binding_name) in [
                ("cmd-ctrl-u", TOGGLE_ACTIVE_TAB_UNREAD_BINDING_NAME),
                ("cmd-ctrl-s", TOGGLE_ACTIVE_TAB_STAR_BINDING_NAME),
                ("cmd-j", JUMP_TO_NEXT_UNREAD_TAB_BINDING_NAME),
            ] {
